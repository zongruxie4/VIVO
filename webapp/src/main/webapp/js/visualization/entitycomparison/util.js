...
//	console.log(processJSONData.currentEntityLevel);
	...
	var entityListTable = $('#datatable').DataTable({
	    ...
	    "oLanguage": {
			...
			"sInfoFiltered": "",
            "oPaginate": {
                "sFirst": i18nStringsUtil.firstString,
                "sPrevious": i18nStringsUtil.previousString,
                "sNext": i18nStringsUtil.nextString,
                "sLast": i18nStringsUtil.lastString
            }
		},
	    ...
	    "fnDrawCallback": function () {
	         ...
	         * */
	        checkIfColorLimitIsReached();
            $("." + dataTableParams.paginationContainerDIVClass).toggle(this.api().page.info().pages > 1);
	    }
	});


	var searchInputBox = $("." + dataTableParams.searchBarParentContainerDIVClass).find("input[type=search]");

	searchInputBox.after("<span id='reset-search' title='" + i18nStringsUtil.clerSearchQuery + "'>X</span>");

	$( document ).on('click', "#reset-search", function() {
		entityListTable.search("").draw();
	});
...
function reloadDataTablePagination(preselectedEntityURIs, jsonData){
	...
	function addNewRowAfterReload(entity) {
		 ...
		 * */
		currentDataTable.row.add(newRow);
	}
	 ...
	 * */
	currentDataTable.page('first').draw(false);
